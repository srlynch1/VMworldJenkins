    ...
    stage('test') {
      steps {
        ...
        sh """export SLACK_USER_TOKEN="${params.slackOauthToken}"
                                        ...
                                        sudo -i -E /home/testrunner/node_modules/.bin/cypress run --spec ${params.testSpecPath} --config video=false --reporter json --env host=http://${params.ipAddress}${params.websiteBase} && echo \$?"""
      }
    }
    stage('Locust') {
      steps {
